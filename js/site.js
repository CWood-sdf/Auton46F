...
var height, p;
var YELLOW, RED, BLUE,
    BLUE_DARK, RED_DARK, rings, PURPLE, POS, POS_TOP_LEFT;
...
var field = {
    ...
    endX: function () { return this.tileWidth * 6 + this.startX; },
...
function drawBot(pt, r, t) {
    ...
    // var botHeight = 14 / 24;
    var upperRightCorner = p.createVector(upperLeftCorner.x + botWidth / 24, upperLeftCorner.y);
    var btmLeftCorner = p.createVector(upperLeftCorner.x, upperLeftCorner.y + botHeight / 24);
    var btmRightCorner = p.createVector(upperLeftCorner.x + botWidth / 24, upperLeftCorner.y + botHeight / 24);
...
function editDriveTo(isBackward) {
    ...
    var angle = botPos.angleTo(fieldPos) * 360 / p.TWO_PI + 90 * (isBackward * -2 + 1);
...
function editFollowPath(isBackward) {
    ...
    var angle = pos.arr[pos.size() - 2].angleTo(pos.last()) * 360 / p.TWO_PI + 90 * (isBackward * -2 + 1);
...
const s = pi => {
    ...
    stage = stages["init"];
    angleSlide = new Slide(10, 96, 50, 2, 360, p.color(0, 0, 0), p.color(255, 0, 0), p.color(255, 255, 0));
    ...
    addBtn = new Button(
        80, 95, 4, 4, p.color(0, 0, 0), p.color(200), p.color(180), p.color(160),
        "+", 0.9, 3.0
    );
