...
const compile = () => {
    var ret = `estimateStartPos(PVector(${limDecimal(pastMvts[0][0].x * FIELD_TO_NORM)}, ${limDecimal(-pastMvts[0][0].y * FIELD_TO_NORM)}), ${limDecimal(pastMvts[0][1])});\n`;
...
function editDriveTo(isBackward) {
    ...
    var angle =
        (botPos.angleTo(fieldPos) * 360) / p.TWO_PI +
        90 * (isBackward * -2 + 1);
    ...
    if (deleteBtn.isDone()) {
        ...
        stage = stages["Programming"];
        var str = `${isBackward ? "backInto" : "driveTo"}(${limDecimal(fieldPos.x * FIELD_TO_NORM)}, ${-limDecimal(fieldPos.y * FIELD_TO_NORM)});`;
        ...
        switch (stage) {
            ...
            case stages["Edit turnTo"]:
                ...
                if (deleteBtn.isDone()) {
                    ...
                    program.push([
                        cmdType["Mvt"],
                        `turnTo(${limDecimal(angle)});`,
                    ]);
                    addListEl(`turnTo(${limDecimal(angle)});`);
                    botAngle = angle;
                    botPos = botCenter;
                }
                break;
            case stages["Edit driveDistance"]:
                ...
                if (deleteBtn.isDone()) {
                    ...
                    program.push([
                        cmdType["Mvt"],
                        `driveDistance(${limDecimal(fieldPos.dist(botPos) * 24)});`,
                    ]);
                    addListEl(
                        `driveDistance(${limDecimal(fieldPos.dist(botPos) * 24)});`,
                    );
            ...
            case stages["Edit backwardsDriveDistance"]:
                ...
                if (deleteBtn.isDone()) {
                    ...
                    program.push([
                        cmdType["Mvt"],
                        `backwardsDriveDistance(${limDecimal(fieldPos.dist(botPos) * 24)});`,
                    ]);
                    addListEl(
                        `backwardsDriveDistance(${limDecimal(fieldPos.dist(botPos) * 24)});`,
                    );
            ...
            case stages["Edit faceTarget"]:
                ...
                if (deleteBtn.isDone()) {
                    ...
                    program.push([
                        cmdType["Mvt"],
                        `faceTarget({${limDecimal(fieldPos.x * FIELD_TO_NORM)}, ${limDecimal(fieldPos.y * -FIELD_TO_NORM)}});`,
                    ]);
                    addListEl(
                        `faceTarget({${limDecimal(fieldPos.x * FIELD_TO_NORM)}, ${limDecimal(fieldPos.y * -FIELD_TO_NORM)}});`,
                    );
